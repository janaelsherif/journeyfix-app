...
/**
 * POST /api/pdf - Generate PDF from scan data (works for free report without DB)
 */
export async function POST(request: NextRequest) {
  try {
    ...
    const tier = reportTier === "BASIC" || reportTier === "ADVANCED" ? reportTier : "FREE";

    const lang = language === "en" ? "en" : "de";
    const t = translations[lang];

    if (!websiteUrl || !useCaseResults || !Array.isArray(useCaseResults)) {
    ...
    const rawResults = useCaseResults as Array<{
      useCaseName: string;
      score: number;
      severity: string;
      briefRemedy: string;
    ...
    const scoreColor =
      (overallScore ?? 0) >= 80 ? "#16a34a" : (overallScore ?? 0) >= 60 ? "#d97706" : "#dc2626";
...
<head>
  ...
  <style>
    ...
    .pdf-table thead { display: table-row-group; }
...
<body>
  <div class="header">
    ...
    <div class="meta">Website: ${websiteUrl}</div>
    <div class="meta">${city ? city + ", " : ""}${canton} • ${profession}</div>
    <div class="score" style="color: ${scoreColor}">${t.pdfOverallScore}: ${overallScore ?? 0}/100</div>
  </div>
  ...
  ${
    tier === "ADVANCED" && statusShiftAnalysis
      ? `
  <div class="section">
    <h2 class="pdf-h2">${t.pdfStatusShift}</h2>
    <div class="pdf-content">${markdownToHtml(statusShiftAnalysis)}</div>
  </div>
  `
      : ""
  }
  ...
  ${
    tier === "ADVANCED" && fiveAngleAnalysis
      ? `
  <div class="section">
    <h2 class="pdf-h2">${t.pdfFiveAngle}</h2>
    <div class="pdf-content">${markdownToHtml(fiveAngleAnalysis)}</div>
  </div>
  `
      : ""
  }
  ...
  ${
    tier === "ADVANCED" && critiqueAnalysis
      ? `
  <div class="section">
    <h2 class="pdf-h2">${t.pdfCritique}</h2>
    <div class="pdf-content">${markdownToHtml(critiqueAnalysis)}</div>
  </div>
  `
      : ""
  }
    ...
    return NextResponse.json({ url: `${baseUrl}/reports/${id}.pdf` });
  } catch (error) {
